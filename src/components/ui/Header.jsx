    ...
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-40 px-4 sm:px-8 py-3.5 transition-all duration-300 pointer-events-none">
        <div className="flex items-center justify-between mx-auto max-w-7xl">
          ...
          <Link
            to="/"
            className="flex items-center gap-3 pointer-events-auto group focus:outline-none"
          >
            ...
            <div className="flex items-center gap-2">
              <div className="flex flex-col">
                <span className="text-sm font-bold leading-tight tracking-tight transition-colors sm:text-base text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400">
          ...
          <div className="flex items-center gap-2 pointer-events-auto md:hidden">
            <button
              onClick={handleOpenSearch}
              className="p-2 transition-colors border shadow-sm rounded-xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-slate-200/80 dark:border-slate-800/80 text-slate-700 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-indigo-400"
              aria-label="Search"
            >
              <Search size={18} />
            </button>
        ...
        {isDrawerOpen && (
          <div className="fixed inset-0 z-50 flex justify-end md:hidden">
            ...
            <motion.div
              ...
              className="relative z-10 flex flex-col justify-between w-full h-full max-w-xs p-6 overflow-y-auto bg-white border-l shadow-2xl dark:bg-slate-900 border-slate-200 dark:border-slate-800"
            >
              {/* Drawer Header */}
              <div>
                <div className="flex items-center justify-between pb-5 mb-6 border-b border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-3">
                    <img
                      src={logoImg}
                      alt="Profile"
                      className="object-cover w-10 h-10 border rounded-xl border-slate-200 dark:border-slate-700"
                    />
                    <div>
                  ...
                  <button
                    onClick={() => setIsDrawerOpen(false)}
                    className="p-2 transition-colors rounded-xl text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800"
                  >
                    <X size={20} />
                  </button>
                </div>
                    ...
                    if (link.isExternal) {
                      return (
                        <a
                          ...
                          href={link.path}
                          onClick={() => setIsDrawerOpen(false)}
                          className="flex items-center gap-3 px-4 py-3 text-sm font-medium transition-all rounded-xl text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60"
                        >
                          <Icon
                            size={18}
                            className={`${link.color} opacity-80 group-hover:opacity-100`}
                          />
                      ...
                      >
                        <Icon
                          ...
                          className={
                            isActive
                              ? link.color
                              : `${link.color} opacity-80 group-hover:opacity-100`
                          }
                        />
                        <span>{link.label}</span>
                      </Link>
              ...
              <div className="pt-6 mt-6 space-y-3 border-t border-slate-100 dark:border-slate-800">
                {/* Book Call CTA */}
                <button
                  onClick={handleBookClick}
                  className="flex items-center justify-center w-full gap-2 py-3 text-sm font-semibold text-white shadow-md rounded-xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 shadow-indigo-500/20"
                >
                  <Calendar size={16} />
